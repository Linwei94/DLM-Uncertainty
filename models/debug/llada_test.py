...
def get_log_likelihood(model, prompt, answer, mc_num=128, batch_size=16, cfg_scale=0., mask_id=126336):
    ...
    return (-np.array(loss_)).tolist()[-len(answer):]  # only return the log likelihood of answer part


def main():
    model = AutoModel.from_pretrained('GSAI-ML/LLaDA-8B-Base', trust_remote_code=True, torch_dtype=torch.bfloat16, device_map="auto").eval()
    ...
    prompt = torch.tensor(tokenizer(prompt)['input_ids']).to(model.device)
    answer = torch.tensor(tokenizer(answer)['input_ids']).to(model.device)
    print(prompt, answer)

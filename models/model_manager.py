...
class ModelManager():
    ...
    def sample(self, prompts, repeat=1):
        if self.model_name == "Meta-Llama-3-8B-Instruct":
            self.model_id = f"meta-llama/{self.model_name}"
            return llama_3.sample(model_id=self.model_id, prompts=prompts, repeat=repeat)
            ...
            return llada_8b.sample(model=self.model_id, prompts=prompts, repeat=repeat)

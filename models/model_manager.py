...
class ModelManager():
    ...
    def sample_4_choices(self, prompts, repeat=1, temperature=0):
        if self.model_name == "Meta-Llama-3-8B-Instruct":
            self.model_id = f"meta-llama/{self.model_name}"
            return llama_3.sample_4_choices(model_id=self.model_id, prompts=prompts, repeat=repeat, temperature=temperature)
            ...
            self.model_id = f"GSAI-ML/{self.model_name}"
            return llada_8b.sample_4_choices(model=self.model_id, prompts=prompts, repeat=repeat, temperature=temperature)

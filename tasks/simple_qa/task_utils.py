...
def preprocess_dataset(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    dataset["question"] = dataset["problem"].values
    dataset["answer"] = dataset["answer"].values
    return dataset
...
# Simple QA-tailored grader 
def simple_qa_llm_grader(cfg: dict, extracted_output: OrganisedOutputs, prompts: PromptCollection, dataset_manager: DatasetsManager = None):
    ...
    for round_outputs in extracted_output.extracted_answers:
        ...
        for question, predicted_answer, correct_answer in zip(questions, round_outputs, correct_answers):
            grading_prompt = GRADER_TEMPLATE.format(
                ...
                predicted_answer="" if predicted_answer is None else predicted_answer
            )
